...
import com.mar.broadcast.alarm.SMSAlarm;
...
import com.mar.mvc.Dispatcher;
import com.mar.schedule.mvc.*;
import com.mar.schedule.mvc.event.*;
import com.mar.socket.SocketLog;
import com.mar.socket.master.MasterWorkerHolder.HeartBeatInfo;
import com.mar.socket.master.reqresp.MasterExecuteJob;
import com.mar.socket.protocol.Protocol.ExecuteKind;
import com.mar.socket.protocol.Protocol.Response;
import com.mar.socket.protocol.Protocol.Status;
import com.mar.store.GroupBean;
import com.mar.store.JobBean;
import com.mar.store.mysql.persistence.JobPersistence;
import com.mar.store.mysql.persistence.JobPersistenceOld;
import com.mar.util.CronExpParser;
...
public class Master {
    ...
    private Map<Long, JobPersistence> actionDetails;
...
//		checkTimeOver();
    ...
    private void runManualJob(MasterWorkerHolder selectWorker, final String jobID) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                jobstatus.setStatus(com.mar.model.JobStatus.Status.RUNNING);
                ...
                try {
                    ...
                    ScheduleInfoLog.error("JobId:" + history.getJobId()
                            + " run failed", e);
                    jobstatus.setStatus(com.mar.model.JobStatus.Status.FAILED);
                    context.getGroupManager().updateJobStatus(jobstatus);
                }
                boolean success = resp.getStatus() == Status.OK ? true : false;
                ...
                if (!success) {
                    ...
                    ScheduleInfoLog.info("manual jobId::" + history.getJobId()
                            + " run success");
                    jobstatus.setStatus(com.mar.model.JobStatus.Status.SUCCESS);
    ...
    private void runScheduleJobContext(MasterWorkerHolder w, final String jobID, int runCount, final int rollBackTimes, final int rollBackWaitTime) {
        ...
        if (runCount == 1) {
            ...
            his.setStatus(com.mar.model.JobStatus.Status.RUNNING);
...
//			SocketLog.info("worker disconnect, ip:" + channel.getRemoteAddress().toString());
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    for (JobHistory his : hiss) {
                        ...
                        his.setStatus(com.mar.model.JobStatus.Status.FAILED);
    ...
    public void debug(DebugHistory debug) {
        ...
        debug.setStatus(com.mar.model.JobStatus.Status.RUNNING);
    ...
    public JobHistory run(JobHistory history) {
        ...
        history.setStatus(com.mar.model.JobStatus.Status.RUNNING);
        ...
        if (history.getStatus() == com.mar.model.JobStatus.Status.RUNNING) {
            history.getLog().appendZeus(
                    new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
            ...
            if (history.getTriggerType() == TriggerType.MANUAL) {
                ...
                JobStatus js = context.getGroupManager().getJobStatus(
                        history.getJobId());
                js.setStatus(com.mar.model.JobStatus.Status.RUNNING);
...
/*									actionPer.setScript(jobDetail.getScript());*/
                                    ...
                                    try {
                                        if (!actionDetails.containsKey(actionPer.getId())) {
                                            ...
                                            //log.info("依赖任务JobId: " + jobDetail.getId()+";  ActionId: " +actionPer.getId());
                                            //if(actionPer.getId()>Long.parseLong(currentDateStr)){
                                            context.getGroupManager().saveJob(actionPer);
                                            //System.out.println("success");
                                            //log.info("success");
