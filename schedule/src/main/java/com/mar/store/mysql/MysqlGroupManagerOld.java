...
import com.mar.store.mysql.tool.JobValidateOld;
...
//			}
	...
	public List<Tuple<JobDescriptorOld, JobStatus>> getChildrenJob(String groupId) {
		List<JobPersistenceOld> list = getHibernateTemplate().find(
				"from com.mar.store.mysql.persistence.JobPersistenceOld where groupId="
						+ groupId);
	...
	public List<JobPersistenceOld> getAllJobs() {
		List<JobPersistenceOld> list = getHibernateTemplate().find(
				"from com.mar.store.mysql.persistence.JobPersistenceOld ");
		return list;
	}
...
/*		String script = persist.getScript();*/
		...
		List<JobPersistence> actionList = template.find("from com.mar.store.mysql.persistence.JobPersistence where toJobId='"+ jobId +"' order by id desc");
