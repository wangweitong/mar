...
import com.mar.model.JobDescriptorOld.JobScheduleTypeOld;
import com.mar.model.JobStatus.Status;
import com.mar.model.JobStatus.TriggerType;
import com.mar.model.processer.Processer;
import com.mar.store.mysql.persistence.*;
import com.mar.util.DateUtil;
...
public class PersistenceAndBeanConvertOld {
	...
	public static JobPersistenceOld convert(Tuple<JobDescriptorOld, JobStatus> job) {
		if (job == null) {
			return null;
		}
		JobPersistenceOld persist = convert(job.getX());
		JobPersistenceOld p2 = convert(job.getY());
	...
	public static Tuple<JobDescriptorOld, JobStatus> convert(JobPersistenceOld persist) {
		if (persist == null) {
			return null;
		}
		...
		jd.setJobType(JobRunTypeOld.parser(persist.getRunType()));
		jd.setScheduleType(JobScheduleTypeOld.parser(persist.getScheduleType()));
		...
		return new Tuple<JobDescriptorOld, JobStatus>(jd, status);
	}

	public static JobPersistenceOld convert(JobStatus jobStatus) {
		if (jobStatus == null) {
			return null;
		}
		JobPersistenceOld persist = new JobPersistenceOld();
	...
	public static JobPersistenceOld convert(JobDescriptorOld jd) {
		if (jd == null) {
			return null;
		}
		JobPersistenceOld persist = new JobPersistenceOld();
