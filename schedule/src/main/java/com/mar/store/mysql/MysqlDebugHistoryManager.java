...
import com.mar.store.DebugHistoryManager;
import com.mar.store.mysql.persistence.DebugHistoryPersistence;
...
public class MysqlDebugHistoryManager extends HibernateDaoSupport implements DebugHistoryManager{
	...
	public int pagingTotal(String fileId) {
		Number number=(Number) getHibernateTemplate().find("select count(*) from com.mar.store.mysql.persistence.DebugHistoryPersistence where fileId="+fileId)
		.iterator().next();
