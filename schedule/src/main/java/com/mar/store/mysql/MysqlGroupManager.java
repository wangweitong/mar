...
import com.mar.model.JobDescriptor.JobScheduleType;
import com.mar.model.JobStatus;
import com.mar.model.processer.DownloadProcesser;
import com.mar.model.processer.Processer;
import com.mar.store.GroupBean;
import com.mar.store.GroupManager;
import com.mar.store.GroupManagerTool;
...
import com.mar.store.mysql.tool.GroupValidate;
import com.mar.store.mysql.tool.JobValidate;
...
//			}
	...
	public List<Tuple<JobDescriptor, JobStatus>> getChildrenJob(String groupId) {
		List<JobPersistence> list = getHibernateTemplate().find(
				"from com.mar.store.mysql.persistence.JobPersistence where groupId="
						+ groupId);
	...
	public List<GroupDescriptor> getChildrenGroup(String groupId) {
		List<GroupPersistence> list = getHibernateTemplate().find(
				"from com.mar.store.mysql.persistence.GroupPersistence where parent="
						+ groupId);
		...
		List<Tuple<JobDescriptor, JobStatus>> list = (List<Tuple<JobDescriptor, JobStatus>>) getHibernateTemplate()
				.execute(new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
						...
						Query query = session
								.createQuery("from com.com.mar.store.mysql.persistence.JobPersistence where id in (:list)");
	...
	public List<JobPersistence> getLastJobAction(String jobId) {
		List<JobPersistence> list = getHibernateTemplate().find(
				"from com.mar.store.mysql.persistence.JobPersistence where toJobId='"+jobId+"' order by id desc");
		return list;
	}
	...
	public boolean IsExistedBelowRootGroup(String GroupName) {
		...
		List<GroupPersistence> tmps = getHibernateTemplate().find("from com.mar.store.mysql.persistence.GroupPersistence where existed=1 and parent=" + rootId);
