...
import com.mar.store.mysql.persistence.JobHistoryPersistence;
...
public class MysqlJobHistoryManager extends HibernateDaoSupport implements JobHistoryManager{
	...
	public int pagingTotal(String jobId) {
		Number number=(Number) getHibernateTemplate().find("select count(*) from com.mar.store.mysql.persistence.JobHistoryPersistence where toJobId="+jobId)
		.iterator().next();
